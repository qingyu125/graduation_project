...
try:
    ...
    RPC_AVAILABLE = False
    print("Warning: RPC components not available")
...
class DeepThinkLLM:
    ...
    def __init__(self, model: str, **vllm_kwargs):
        ...
        # Extract RPC parameters before passing to vLLM
        rpc_enabled = vllm_kwargs.get('enable_rpc', False)
        rpc_params = {
            'enable_rpc': rpc_enabled,
            'rpc_P': vllm_kwargs.get('rpc_P', 1024),
            'rpc_R': vllm_kwargs.get('rpc_R', 32),
            'rpc_c': vllm_kwargs.get('rpc_c', 4),
            'rpc_selectors': vllm_kwargs.get('rpc_selectors', 'recent'),
            'rpc_aggregation': vllm_kwargs.get('rpc_aggregation', 'all'),
            'rpc_kernel_size': vllm_kwargs.get('rpc_kernel_size', 7),
            'rpc_pooling': vllm_kwargs.get('rpc_pooling', 'avgpool'),
        }
        
        # Filter out RPC parameters from vLLM kwargs
        rpc_param_names = ['enable_rpc', 'rpc_P', 'rpc_R', 'rpc_c', 'rpc_selectors', 
                          'rpc_aggregation', 'rpc_kernel_size', 'rpc_pooling']
        vllm_only_kwargs = {k: v for k, v in vllm_kwargs.items() if k not in rpc_param_names}
        ...
        default_kwargs.update(vllm_only_kwargs)
        
        print("Initializing vLLM engine...")
        llm_init_start = time.time()
        
        # Enable RPC if available and requested
        if rpc_enabled and RPC_AVAILABLE:
            print("Enabling RPC...")
            enable_rpc()
        
        self.llm = LLM(model=model, logits_processors=[WrappedPerReqLogitsProcessor], **default_kwargs)
        
        # Configure RPC if enabled
        if rpc_enabled and RPC_AVAILABLE:
            rpc_config = {
                'P': rpc_params['rpc_P'],
                'R': rpc_params['rpc_R'],
                'c': rpc_params['rpc_c'],
                'selectors': rpc_params['rpc_selectors'],
                'aggregation': rpc_params['rpc_aggregation'],
                'kernel_size': rpc_params['rpc_kernel_size'],
                'pooling': rpc_params['rpc_pooling'],
            }
            set_rpc_config(self.llm, **rpc_config)
            self.rpc_enabled = True
            self.rpc_config = rpc_config
            print(f"RPC configured with: {rpc_config}")
        else:
