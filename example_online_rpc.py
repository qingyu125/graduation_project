"""
Example usage of DeepThinkLLM in online mode with RPC - processes a single question
...
from deepconf_rpc import DeepThinkLLM
...
def prepare_prompt(question: str, tokenizer, model_type: str = "deepseek") -> str:
    ...
    if model_type == "deepseek":
        ...
        messages = [
            ...
            # {"role": "user", "content": f"{question}\nPlease answer step by step and demonstrate the reasoning process in detail."}  # 显式添加引导
        ]
    else:
...
def print_evaluation_report(question, ground_truth, evaluation, confidence_eval, result):
    ...
    # RPC information
    if hasattr(result, 'rpc_enabled') and result.rpc_enabled:
        print(f"RPC Enabled: True")
        print(f"RPC Config: {result.rpc_config}")
    else:
        print(f"RPC Enabled: False")
...
def main():
    parser = argparse.ArgumentParser(description='DeepThinkLLM Online Mode with RPC Example')
    parser.add_argument('--model', type=str, default="./models/DeepSeek-R1-0528-Qwen3-8B",
    ...
    parser.add_argument('--rid', type=str, default="online_rpc_run",
    ...
    parser.add_argument('--output_dir', type=str, default="rpc_outputs",
    ...
    # RPC parameters
    parser.add_argument('--enable_rpc', action='store_true', default=True,
                       help='Enable RPC compression')
    ...
    # Initialize DeepThinkLLM with RPC
    deep_llm = DeepThinkLLM(model=args.model, **vllm_kwargs)
    ...
    # Run deep thinking in online mode with RPC
    ...
    # Add RPC information to result
    result.rpc_enabled = args.enable_rpc
    result.rpc_config = {
        'P': args.rpc_P,
        'R': args.rpc_R,
        'c': args.rpc_c,
        'selectors': args.rpc_selectors,
        'aggregation': args.rpc_aggregation,
        'kernel_size': args.rpc_kernel_size,
        'pooling': args.rpc_pooling,
    }
    ...
    result_data.update({
        ...
        'rpc_enabled': args.enable_rpc,
        'rpc_config': result.rpc_config,
    })
    
    result_filename = f"{args.output_dir}/deepthink_online_rpc_qid{args.qid}_rid{args.rid}_{timestamp}.pkl"
